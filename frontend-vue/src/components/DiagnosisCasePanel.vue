...
provide(sqlPreviewContextKey, {
  token: toRef(props, 'token'),
  ruleId: computed(() => props.snapshot.ruleId),
  profileId: computed(() => props.snapshot.profileId),
  statStart: computed(() => String(props.snapshot.caseInput.statStart || '')),
  statEnd: computed(() => String(props.snapshot.caseInput.statEnd || '')),
})
...
const issueDirection = ref<'OVER_INCLUDED' | 'UNDER_INCLUDED' | 'WRONG_CLASSIFICATION' | 'SUSPECT_SYNC' | 'UNKNOWN'>('UNKNOWN')
...
const issueDirectionOptions = [
  { value: 'OVER_INCLUDED', label: '多算了', help: '本来不应统计，但出现在结果或明细中' },
  { value: 'UNDER_INCLUDED', label: '少算了', help: '本来应该统计，但结果或明细中没有' },
  { value: 'WRONG_CLASSIFICATION', label: '归类不对', help: '已经统计，但分子、分母或状态判定不对' },
  { value: 'SUSPECT_SYNC', label: '怀疑未同步', help: '重新抽取后可能恢复，先核对业务源与中间数据' },
  { value: 'UNKNOWN', label: '还不确定', help: '先逐层核对，再判断问题发生在哪一步' },
] as const
const submittedIssueDirection = computed(() => String(props.snapshot.caseInput.issueDirection || 'UNKNOWN'))
const submittedIssueDirectionLabel = computed(() => issueDirectionOptions.find((item) => item.value === submittedIssueDirection.value)?.label || '还不确定')
...
function submitCase() {
  ...
  emit('action', 'SUBMIT_CASE', {
    issueDirection: issueDirection.value,
...
async function loadScopeClarification(force = false) {
  ...
  if (!['RECORD', 'DEPARTMENT', 'TIME_RANGE', 'DATA_CATEGORY', 'OVERALL'].includes(type)) return
  const key = [
    ...
    props.snapshot.caseInput.issueDirection,
    props.snapshot.caseInput.scopeStart,
    props.snapshot.caseInput.scopeEnd,
  ].join(':')
...
watch(
  ...
  () => {
    if (caseSubmitted.value
      && ['RECORD', 'DEPARTMENT', 'TIME_RANGE', 'DATA_CATEGORY', 'OVERALL'].includes(String(props.snapshot.caseInput.scopeType || ''))) {
...
<template>
  <div class="diagnosis-case-thread" :data-status="snapshot.status">
    ...
    <template v-if="caseInputReached && snapshot.investigationMode !== 'AUTONOMOUS' && selectedMode === 'STANDARD'">
      <article class="message is-agent"><div class="message-avatar">AI</div><div class="message-card diagnosis-turn-card"><div class="message-head"><strong>系统 · 说明问题方向和排查范围</strong></div><p>先说明是多算、少算、归类不对还是怀疑没有同步，再选择患者、科室、时间或整体范围。系统会按业务源、指标中间数据、分母、分子的顺序逐层核对。</p><p class="diagnosis-help">具体患者编号只在排查某条记录时需要；科室漏数和整体差异不要求患者编号。</p></div></article>
      <article v-if="snapshot.currentStep === 'CASE_INPUT'" class="message is-user"><div class="message-avatar">我</div><div class="message-card diagnosis-turn-card"><div class="message-head"><strong>实施人员 · 排查范围</strong></div>
        <section class="diagnosis-direction-picker">
          <h4>现在看到的现象</h4>
          <div class="diagnosis-direction-options" role="radiogroup" aria-label="问题方向">
            <button v-for="item in issueDirectionOptions" :key="item.value" type="button" :class="{ active: issueDirection === item.value }" @click="issueDirection = item.value"><strong>{{ item.label }}</strong><span>{{ item.help }}</span></button>
          </div>
        </section>
        <h4 class="diagnosis-scope-heading">问题发生在哪个范围</h4>
        ...
        <div class="diagnosis-case-grid">
          ...
          <label class="wide">医院认为应该是什么结果{{ scopeType === 'OVERALL' ? '' : '（可选）' }}<textarea v-model="caseDescription" rows="3" maxlength="1200" :placeholder="scopeType === 'OVERALL' ? '例如：医院同期约500人次，系统只有417人次' : '例如：这名患者应该进入分母但没有出现；不知道可以暂不填写。'"></textarea></label>
        </div>
    ...
    <template v-if="caseSubmitted">
      <article class="message is-user"><div class="message-avatar">我</div><div class="message-card diagnosis-turn-card"><div class="message-head"><strong>实施人员 · 排查范围</strong><span>{{ submittedIssueDirectionLabel }}</span></div><p>{{ submittedScopeSummary }}</p><p v-if="snapshot.caseInput.caseDescription && String(snapshot.caseInput.scopeType || '') !== 'OVERALL'"><strong>医院预期：</strong>{{ snapshot.caseInput.caseDescription }}</p></div></article>
    ...
    <article
      v-if="caseSubmitted && ['RECORD', 'DEPARTMENT', 'TIME_RANGE', 'DATA_CATEGORY', 'OVERALL'].includes(String(snapshot.caseInput.scopeType || ''))"
          ...
          <section class="diagnosis-evidence-journey">
            ...
            <div class="diagnosis-evidence-stages">
                ...
                <div>
                  ...
                  <details v-if="extractionEvidenceHasDetails" class="diagnosis-technical">
                    ...
                    <section v-for="stage in extractionEvidenceStages" :key="stage.stageKey" class="diagnosis-extraction-side">
                      ...
                      <SqlExecuteButton v-if="stage.sql" :token="token" :sql="stage.sql" :database-role="String(stage.databaseRole || '')" :rule-id="snapshot.ruleId" :profile-id="snapshot.profileId" :stat-start="scopeClarification.statStart" :stat-end="scopeClarification.statEnd" compact />
              ...
              <article v-for="stage in (extractionCountsMatch ? [] : extractionEvidenceStages)" :key="stage.stageKey" :data-state="stage.status">
                <div class="diagnosis-stage-marker"><span></span></div>
                <div><strong>{{ stage.label }}</strong><em>{{ evidenceStageStatusText(stage.status) }}</em><p>{{ stage.meaning }}</p><small v-if="stage.count !== undefined">记录数：{{ stage.count }}</small><details v-if="stage.sampleRows?.length || stage.sql || stage.error" class="diagnosis-technical"><summary>查看这一环节的查询与部分记录</summary><DetailRowsTable v-if="stage.sampleRows?.length" :rows="stage.sampleRows" empty-text="这一环节没有找到记录。" /><SqlExecuteButton v-if="stage.sql" :token="token" :sql="stage.sql" :database-role="String(stage.databaseRole || '')" :rule-id="snapshot.ruleId" :profile-id="snapshot.profileId" :stat-start="scopeClarification.statStart" :stat-end="scopeClarification.statEnd" compact /><pre v-if="stage.sql">{{ stage.sql }}</pre><p v-if="stage.error" class="indicator-error">{{ stage.error }}</p></details></div>
              </article>
    ...
    <template v-if="investigationReached">
      ...
      <template v-if="pendingRequirementVisible() && pendingRequirement">
        <article class="message is-user"><div class="message-avatar">我</div><div class="message-card diagnosis-turn-card"><div class="message-head"><strong>实施人员 · 排查要求</strong><span>{{ pendingRequirement.layerLabel }}</span></div><p>{{ pendingRequirement.requirement }}</p><details v-if="pendingRequirement.validationSql" class="diagnosis-technical"><summary>查看实施验证 SQL</summary><SqlExecuteButton :token="token" :sql="pendingRequirement.validationSql" :database-role="pendingRequirement.layer === 'SOURCE_EXTRACT' ? 'BUSINESS' : 'REAL'" :rule-id="snapshot.ruleId" :profile-id="snapshot.profileId" :stat-start="snapshot.caseInput.statStart" :stat-end="snapshot.caseInput.statEnd" compact /><pre>{{ pendingRequirement.validationSql }}</pre></details></div></article>
      ...
      <template v-for="item in snapshot.evidence" :key="String(item.evidenceId)">
        <article class="message is-user"><div class="message-avatar">我</div><div class="message-card diagnosis-turn-card">
          <template v-if="isRequirementEvidence(item)"><div class="message-head"><strong>实施人员 · 排查要求</strong><span>{{ record(item.sqlContext).layerLabel || '暂不确定' }}</span></div><p>{{ item.requirement }}</p><details v-if="item.validationSql" class="diagnosis-technical"><summary>查看实施验证 SQL</summary><SqlExecuteButton :token="token" :sql="String(item.validationSql || '')" :database-role="String(record(item.sqlContext).layer || '') === 'SOURCE_EXTRACT' ? 'BUSINESS' : 'REAL'" :rule-id="snapshot.ruleId" :profile-id="snapshot.profileId" :stat-start="snapshot.caseInput.statStart" :stat-end="snapshot.caseInput.statEnd" compact /><pre>{{ item.validationSql }}</pre></details></template>
    ...
    <template v-if="snapshot.currentStep === 'SHADOW_TRIAL' || snapshot.currentStep === 'DRAFT_SAVE'">
      <article class="message is-agent"><div class="message-avatar">AI</div><div class="message-card diagnosis-turn-card"><div class="message-head"><strong>系统 · 候选语句已生成</strong></div><p>候选语句已通过安全校验，系统随后会自动执行影子试跑，无需再点击单独的“试跑”按钮。正式中间表和当前卡片不会被覆盖。</p><p v-if="snapshot.candidateSql.generationMethod" class="diagnosis-help">生成方式：{{ snapshot.candidateSql.generationMethod }}</p>
        ...
        <details v-if="Object.keys(snapshot.candidateSql).length" class="diagnosis-technical diagnosis-sql-disclosure"><summary><span>当前正式{{ sqlLayerTitle }}（可复制到 Navicat）</span><span class="diagnosis-sql-actions"><button type="button" class="diagnosis-copy-button" @click.prevent.stop="copySql('original', snapshot.candidateSql.originalSqlExecutable)">{{ copiedSqlKey === 'original' ? '已复制' : '复制 SQL' }}</button><SqlExecuteButton :token="token" :sql="String(snapshot.candidateSql.originalSqlExecutable || '')" :database-role="String(snapshot.candidateSql.layer || '') === 'SOURCE_EXTRACT' ? 'BUSINESS' : 'REAL'" :rule-id="snapshot.ruleId" :profile-id="snapshot.profileId" :stat-start="snapshot.caseInput.statStart" :stat-end="snapshot.caseInput.statEnd" compact /></span></summary><pre>{{ executableSqlDisplay(String(snapshot.candidateSql.originalSqlExecutable || '')) }}</pre></details>
        <details v-if="Object.keys(snapshot.candidateSql).length" class="diagnosis-technical diagnosis-sql-disclosure"><summary><span>候选{{ sqlLayerTitle }}（影子试跑版）</span><span class="diagnosis-sql-actions"><button type="button" class="diagnosis-copy-button" @click.prevent.stop="copySql('candidate', snapshot.candidateSql.candidateSqlExecutable)">{{ copiedSqlKey === 'candidate' ? '已复制' : '复制 SQL' }}</button><SqlExecuteButton :token="token" :sql="String(snapshot.candidateSql.candidateSqlExecutable || '')" :database-role="String(snapshot.candidateSql.layer || '') === 'SOURCE_EXTRACT' ? 'BUSINESS' : 'REAL'" :rule-id="snapshot.ruleId" :profile-id="snapshot.profileId" :stat-start="snapshot.caseInput.statStart" :stat-end="snapshot.caseInput.statEnd" compact /></span></summary><pre>{{ executableSqlDisplay(String(snapshot.candidateSql.candidateSqlExecutable || '')) }}</pre></details><p class="diagnosis-help"><strong>可执行版说明：</strong>脚本不写死数据库名和 Schema；请先在 Navicat 中选择正确数据库后再执行。保存医院草稿时仍写入知识库模板 SQL。</p><p class="diagnosis-help">{{ snapshot.candidateSql.rawSqlNotice }}</p>
    ...
    <section v-if="snapshot.currentStep === 'COMPLETED' && Object.keys(snapshot.draftResult || {}).length" class="diagnosis-sql-run-shortcuts"><strong>只读执行草稿 SQL</strong><span><SqlExecuteButton :token="token" :sql="String(snapshot.draftResult.originalSql || '')" :database-role="String(snapshot.draftResult.changeLayer || '') === 'SOURCE_EXTRACT' ? 'BUSINESS' : 'REAL'" :rule-id="snapshot.ruleId" :profile-id="snapshot.profileId" :stat-start="snapshot.caseInput.statStart" :stat-end="snapshot.caseInput.statEnd" compact /><SqlExecuteButton :token="token" :sql="String(snapshot.draftResult.candidateSql || '')" :database-role="String(snapshot.draftResult.changeLayer || '') === 'SOURCE_EXTRACT' ? 'BUSINESS' : 'REAL'" :rule-id="snapshot.ruleId" :profile-id="snapshot.profileId" :stat-start="snapshot.caseInput.statStart" :stat-end="snapshot.caseInput.statEnd" compact /></span></section>
