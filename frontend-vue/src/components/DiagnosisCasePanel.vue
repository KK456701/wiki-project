...
function submitCase() {
  if (!recordId.value.trim() || !caseDescription.value.trim()) return
...
<template>
  <div class="diagnosis-case-thread" :data-status="snapshot.status">
    ...
    <template v-if="caseSubmitted">
      <article class="message is-user"><div class="message-avatar">我</div><div class="message-card diagnosis-turn-card"><div class="message-head"><strong>实施人员 · 具体案例</strong></div><p>{{ snapshot.caseInput.recordField }}={{ snapshot.caseInput.recordId }}</p><p>{{ snapshot.caseInput.caseDescription || snapshot.caseInput.symptom }}</p></div></article>
