...
async function addAlias() {
  ...
  await adminOperation(async () => {
    await createTerminologyAlias(store.token, {
...
async function approveAlias(id: unknown) {
  ...
  await adminOperation(() => approveTerminologyAlias(store.token, Number(id)), '候选词已批准。')
}

async function addMapping() {
  if (!detail.value || !store.user) return
  await adminOperation(async () => {
    await createTerminologyMapping(store.token, {
      hospitalId: store.user?.hospitalId, conceptCode: detail.value?.conceptCode,
...
async function approveMapping(id: unknown) {
  await adminOperation(() => approveTerminologyMapping(store.token, Number(id)), '本院映射已批准。')
}

async function publish() {
  if (!window.confirm('确认发布全部已审核公司术语？发布后将进入识别链路。')) return
  await adminOperation(() => publishTerminology(store.token), '术语版本已发布。', true)
}

async function restore(releaseId: unknown) {
  if (!window.confirm(`确认回退到 ${String(releaseId)}？历史版本不会删除。`)) return
  await adminOperation(() => restoreTerminology(store.token, String(releaseId)), '术语版本已恢复。', true)
}
...
<template>
  <main class="term-shell">
    ...
    <p v-if="operationMessage" class="term-hint">{{ operationMessage }}</p>
    ...
    <section class="term-layout">
      ...
      <article v-if="detail" class="term-detail">
        ...
        <section class="term-detail-grid">
          ...
          <article><h3>术语版本</h3><button class="term-publish" type="button" :disabled="adminBusy" @click="publish">发布已审核术语</button><div class="term-record" v-for="release in releases" :key="String(release.releaseId)"><div><strong>v{{ release.version }} · {{ release.releaseId }}</strong><small>{{ release.changeSummary || '术语发布' }}</small></div><p><span>{{ release.status === 'active' ? '当前生效' : '历史版本' }}</span><button v-if="release.status !== 'active'" type="button" @click="restore(release.releaseId)">回退</button></p></div><p v-if="!releases.length">尚无发布版本。</p></article>
        </section>
      </article>
