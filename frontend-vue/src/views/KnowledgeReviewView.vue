    ...
    <section class="knowledge-review-workspace">
      ...
      <article class="knowledge-draft-detail">
        <template v-if="selected">
          ...
          <details class="knowledge-sql"><summary><span>当前正式{{ selected.changeLayer === 'SOURCE_EXTRACT' ? '源表抽取 SQL' : '目标表概览 SQL' }}（知识库模板）</span><span class="sql-action-pair"><button type="button" @click.prevent.stop="copySql('original', selected.originalSql)">{{ copied === 'original' ? '已复制' : '复制 SQL' }}</button><SqlExecuteButton :token="store.token" :sql="selected.originalSql" :database-role="selected.changeLayer === 'SOURCE_EXTRACT' ? 'BUSINESS' : 'REAL'" :rule-id="selected.ruleId" :profile-id="selected.profileId" :stat-start="String(selected.changeRequest.statStart || selected.shadowTrial.statStart || '')" :stat-end="String(selected.changeRequest.statEnd || selected.shadowTrial.statEnd || '')" compact /></span></summary><pre>{{ selected.originalSql }}</pre></details>
          <details class="knowledge-sql"><summary><span>待审批候选{{ selected.changeLayer === 'SOURCE_EXTRACT' ? '源表抽取 SQL' : '目标表概览 SQL' }}（知识库模板）</span><span class="sql-action-pair"><button type="button" @click.prevent.stop="copySql('candidate', selected.candidateSql)">{{ copied === 'candidate' ? '已复制' : '复制 SQL' }}</button><SqlExecuteButton :token="store.token" :sql="selected.candidateSql" :database-role="selected.changeLayer === 'SOURCE_EXTRACT' ? 'BUSINESS' : 'REAL'" :rule-id="selected.ruleId" :profile-id="selected.profileId" :stat-start="String(selected.changeRequest.statStart || selected.shadowTrial.statStart || '')" :stat-end="String(selected.changeRequest.statEnd || selected.shadowTrial.statEnd || '')" compact /></span></summary><pre>{{ selected.candidateSql }}</pre></details>
