...
const aiPatientOptions = ref<Array<{ value: string; label: string }>>([])
const aiSelectedPatients = ref<Array<{ value: string; label: string }>>([])
...
const screeningGroups = computed(() => {
  const groups = new Map<string, { label: string; findings: DiagnosisDataScreening['findings'] }>()
  for (const finding of visibleScreeningFindings.value) {
    const field = String(finding.field || '').toUpperCase()
    const key = finding.ruleCode === 'DETERMINISTIC_DUPLICATE_KEY' ? 'duplicate'
      : field.includes('DEPT') || field.includes('WARD') ? 'department'
        : field.includes('NAME') || field.includes('PERSON') || field.includes('PATIENT') ? 'patient' : 'other'
    const label = key === 'department' ? '科室名称含「测试 / test」'
      : key === 'patient' ? '患者姓名含「测试 / test」'
        : key === 'duplicate' ? '同一业务编号重复出现' : '明细字段含明确测试标记'
    const group = groups.get(key) || { label, findings: [] }
    group.findings.push(finding)
    groups.set(key, group)
  }
  return [...groups.values()].map((group) => ({
    ...group,
    totalCount: group.findings.length,
    findings: screeningExpanded.value ? group.findings : group.findings.slice(0, 3),
  }))
})
...
const overallButtonLabel = computed(() => {
  ...
  return hasPendingModification.value ? '整体执行候选链路' : '整体执行当前链路'
})
...
function toggleRow(row: Record<string, unknown>) {
  ...
  else copy.set(key, { ...row, __sourceGroup: detailGroup.value })
...
function screeningFindingLine(finding: DiagnosisDataScreening['findings'][number]): string {
  const row = finding.row || {}
  const name = row.FULL_NAME || row.PERSON_NAME || row.personName || '未登记姓名'
  const encounter = row.ENCOUNTER_ID || row.encounterId || row.BIZ_ID || row.bizId || finding.rowKey
  const dept = row.CURRENT_DEPT_NAME || row.currentDeptName || row.CURRENT_WARD_NAME || row.currentWardName || '未登记科室'
  return `${String(name)}（${String(encounter)} · ${String(dept)}）`
}
...
async function searchAiPatients() {
  ...
  try {
    ...
    pages.flatMap((page) => page.rows || []).forEach((row) => {
      ...
      if (value) unique.set(value, { value, label: `${displayRowLabel(row)}（${value}）` })
    })
...
function toggleAiPatient(option: { value: string; label: string }) {
...
function importConfirmationScope() {
  ...
  confirmationRows().forEach((item) => patientMap.set(item.recordId, {
    value: item.recordId, label: `${item.label}（${item.recordId}）`,
  }))
  aiSelectedPatients.value = [...patientMap.values()]
  aiSelectedDepartments.value = [...new Set([...aiSelectedDepartments.value, ...selectedDepartments.value])]
}
...
async function clarifyData(direction: 'OVER_INCLUDED' | 'UNDER_INCLUDED') {
  await act('CLARIFY_DATA_CONFIRMATION', {
    direction,
    targets: clarificationTargets(direction),
    description: direction === 'OVER_INCLUDED' ? overIncludedNote.value.trim() : underIncludedNote.value.trim(),
  })
}
...
async function executeWholeLineage() {
  ...
  if (['SHADOW_TRIAL', 'DRAFT_SAVE'].includes(String(snapshot.value.currentStep))
    ...
    trialProgress.value = '正在执行候选影子链路'
    const completed = await act('RUN_SHADOW_TRIAL', {})
    if (!completed) {
      trialError.value = error.value || '影子试跑请求失败。'
      trialProgress.value = '试跑失败'
    } else {
      trialProgress.value = Boolean(completed.shadowTrial?.passed) ? '试跑完成' : '试跑失败'
    }
    return
  }
...
<template>
  <main class="standard-diagnosis-workspace">
    ...
    <section v-else-if="currentStep === 'selection'" class="workspace-page selection-page">
      <header class="page-heading compact-heading"><h1>选择指标、时间与本次口径</h1></header>
    ...
    <section v-else-if="currentStep === 'data'" class="workspace-page data-page">
      <header class="page-heading data-heading"><div><h1>指标统计数据 · 确认是否缺失或增多</h1></div><em>{{ String(snapshot?.caseInput.statStart || '—').slice(0, 10) }} 至 {{ String(snapshot?.caseInput.statEnd || '—').slice(0, 10) }}</em></header>
      <div v-if="checksPreparing" class="background-check-status"><span class="trial-spinner" aria-hidden="true"></span><div><strong>正在准备本次统计数据</strong><p>{{ String(gate(currentGateNumber(snapshot?.currentStep || ''))?.message || '系统正在核对数据结构、事件抽取和当前统计窗口。') }}</p></div></div>
      ...
      <template v-else>
        <section class="statistics-summary-card">
          <header><div><span>本次统计结论</span><strong>{{ selectedIndicator?.ruleName || snapshot?.caliberSnapshot.ruleName || '当前指标' }}</strong></div><em>所选口径的正式统计结果</em></header>
          <div class="metric-strip statistics-card"><article class="primary-result"><span>指标结果</span><strong>{{ resultValue }}</strong><small>当前口径计算值</small></article><button type="button" :class="{ active: detailGroup === 'numerator' }" @click="toggleDetails('numerator')"><span>统计 SQL 分子</span><strong>{{ numeratorCount }}</strong><small>{{ detailGroup === 'numerator' ? '收起分子明细 ↑' : '点击查看分子明细 →' }}</small></button><button type="button" :class="{ active: detailGroup === 'denominator' }" @click="toggleDetails('denominator')"><span>统计 SQL 分母</span><strong>{{ denominatorCount }}</strong><small>{{ detailGroup === 'denominator' ? '收起分母明细 ↑' : '点击查看分母明细 →' }}</small></button></div>
        </section>
        ...
        <section v-if="detailGroup" class="detail-browser is-expanded">
          ...
          <div v-else class="detail-table-wrap"><table><thead><tr><th>数据多了</th><th v-for="column in detailColumns" :key="column"><span>{{ detailFieldLabel(column) }}</span><small v-if="detailFieldLabel(column) !== column">{{ column }}</small></th></tr></thead><tbody><tr v-for="row in detailPage?.rows || []" :key="rowKey(row)" :class="{ selected: selectedRows.has(rowKey(row)) }"><td><input type="checkbox" :checked="selectedRows.has(rowKey(row))" aria-label="标记为不应统计" @change="toggleRow(row)" /></td><td v-for="column in detailColumns" :key="column">{{ formatDetailValue(column, row[column]) }}</td></tr></tbody></table><p v-if="!detailPage?.rows?.length" class="workspace-empty">当前筛选条件下没有明细。</p></div>
        ...
        <section class="screening-panel compact-screening" :class="{ 'has-findings': Boolean(screening?.findingCount) }"><header><div><strong>🤖 AI初筛</strong><span v-if="screening?.findingCount" class="screening-count">疑似测试或重复数据 {{ screening.findingCount }} 条</span></div><button v-if="(screening?.findingCount || 0) > 3" type="button" class="screening-toggle" @click="screeningExpanded = !screeningExpanded">{{ screeningExpanded ? '收起全部' : `查看全部 ${screening?.findingCount} 条` }}</button></header><p v-if="screeningLoading">正在检查患者姓名、科室名称和重复业务编号…</p><p v-else-if="!screening?.findingCount">当前明细中未发现明确的“测试 / test”字样或重复业务编号。</p><p v-else class="screening-guidance">点击记录即可加入下方“已选择范围”；收起分组不会改变真实总数或已选状态。</p><div v-for="group in screeningGroups" :key="group.label" class="screening-group"><header><strong>{{ group.label }}</strong><span>{{ group.totalCount }} 条</span></header><button v-for="finding in group.findings" :key="finding.findingId" type="button" :class="{ selected: selectedRows.has(finding.rowKey) }" @click="findingSelect(finding)"><span>{{ screeningFindingLine(finding) }}</span><small>{{ selectedRows.has(finding.rowKey) ? '✓ 已加入排除范围' : finding.reason }}</small></button></div></section>
    ...
    <section v-else class="workspace-page lineage-page">
      <header class="lineage-toolbar"><div><strong>{{ snapshot?.caliberSnapshot.ruleName }} · {{ snapshot?.caliberSnapshot.profileName }}</strong></div><div class="lineage-actions"><button type="button" class="workspace-primary" title="没有修改时核验当前正式链路；有修改时核验候选链路，全程不修改正式数据" :disabled="!overallExecutionReady" @click="executeWholeLineage"><span v-if="busy || checksPreparing" class="trial-spinner" aria-hidden="true"></span>{{ overallButtonLabel }}</button></div></header>
      <div class="lineage-layout">
        <aside class="flow-rail"><header><strong>当前生效数据链路</strong><span>{{ flowNodes.length }} 个节点</span></header><button v-for="node in flowNodes" :key="String(node.id)" type="button" :class="{ active: String(node.id) === selectedNodeId, editable: ['SOURCE_EXTRACT_SQL', 'OVERVIEW_SQL'].includes(String(node.nodeType)) }" @click="selectNode(node)"><b>{{ String(node.sequence || '').padStart(2, '0') }}</b><span><strong>{{ String(node.title || '未命名节点') }}</strong><small>{{ databaseLabel(node.databaseRole) }}</small></span><em v-if="String(shadow.nodeId || candidate.nodeId || '') === String(node.id)" class="trial-badge">✓ 已试跑</em><em v-else>{{ ['SOURCE_EXTRACT_SQL', 'OVERVIEW_SQL'].includes(String(node.nodeType)) ? '✎ 可修改' : '◉ 只读' }}</em></button><p v-if="!flowNodes.length" class="workspace-empty">当前知识库没有可展示的数据链路。</p></aside>
        <section v-if="selectedNode" class="node-inspector">
          <header class="node-heading" :class="{ editable: selectedNodeEditable }"><div><h1>{{ String(selectedNode.title || '未命名节点') }}</h1><p>{{ String(selectedNode.description || '') }}</p></div><em>{{ databaseLabel(selectedNode.databaseRole) }} · {{ selectedNodeEditable ? '可修改' : '只读' }}</em></header>
          <section v-if="strings(selectedNode.tableNames).length" class="node-table-section"><header><strong>这一环节用到的数据表</strong><span>以下是 SQL 实际读取或写入的表，以及每张表在本指标中的用途。</span></header><ul class="node-table-list"><li v-for="table in strings(selectedNode.tableNames)" :key="table"><strong>{{ table }}</strong><span>{{ tablePurpose(table, record(selectedNode.tableDescriptions)[table]) }}</span></li></ul></section>
          <details v-if="String(selectedNode.sql || '').trim()" class="sql-template-evidence"><summary>查看当前正式 SQL</summary><section class="sql-panel"><header><strong>当前统计窗口可直接执行 SQL</strong><button type="button" @click="copyText(`node-${String(selectedNode.id)}`, selectedNode.sql)">{{ copiedKey === `node-${String(selectedNode.id)}` ? '已复制' : '复制 SQL' }}</button></header><pre>{{ String(selectedNode.sql) }}</pre></section></details>
          <p v-if="!selectedNodeEditable" class="node-readonly-note">该节点用于查看数据从哪里来、流向哪里，不在这里修改 SQL。</p>
          ...
          <section v-else-if="selectedNodeEditable && ['CASE_INPUT', 'CASE_INVESTIGATION'].includes(String(snapshot?.currentStep))" class="candidate-editor">
            <header><strong>修改{{ selectedNodeLayer === 'SOURCE_EXTRACT' ? '抽取' : '概览' }} SQL</strong><span>只在影子环境执行</span></header>
            ...
            <template v-else>
              ...
              <details class="ai-scope-picker">
                <summary><span><strong>选择排查患者或科室</strong><small>患者 {{ aiSelectedPatients.length }} 位 · 科室 {{ aiSelectedDepartments.length }} 个</small></span><em>展开选择</em></summary>
                ...
                <section><label><span>搜索患者</span><div><input v-model="aiPatientSearch" type="search" placeholder="输入姓名、就诊号或住院号" @keyup.enter="searchAiPatients" /><button type="button" :disabled="aiScopeLoading" @click="searchAiPatients">{{ aiScopeLoading ? '查询中…' : '查询' }}</button></div></label><div class="ai-scope-options"><label v-for="item in aiPatientOptions" :key="item.value"><input type="checkbox" :checked="aiSelectedPatients.some((selected) => selected.value === item.value)" @change="toggleAiPatient(item)" /><span>{{ item.label }}</span></label><p v-if="aiPatientOptions.length === 0">输入关键词后查询本次分子、分母明细。</p></div></section>
                <section><label><span>选择科室</span><input v-model="aiDepartmentSearch" type="search" placeholder="搜索科室名称或编码" /></label><div class="ai-scope-options"><label v-for="item in filteredAiDepartments" :key="item.value"><input type="checkbox" :checked="aiSelectedDepartments.includes(item.value)" @change="toggleAiDepartment(item.value)" /><span>{{ item.label }}</span><small>分母 {{ item.denominatorCount }} · 分子 {{ item.numeratorCount }}</small></label></div></section>
              </details>
              <p class="candidate-helper">提交时，文字要求和上面选择的患者、科室会一起交给程序校验。查询成功后才会生成影子数据和对账结果。</p>
            </template>
