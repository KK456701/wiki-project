...
import {
  loadMonitoringAlerts, loadMonitoringPlans, loadMonitoringResults, loginAdmin, logoutAdmin,
  saveMonitoringPlan, setMonitoringPlanStatus, transitionMonitoringAlert,
  type MonitoringAlert, type MonitoringPlan, type MonitoringResult,
} from '../api/agent'
...
async function refresh() {
  ...
  try {
    ...
    plans.value = p.items; results.value = r.items; alerts.value = a.items
...
<template>
  <main class="monitor-shell">
    ...
    <template v-else>
      <section class="monitor-context"><strong>{{ store.user?.hospitalId }}</strong><span>{{ plans.length }} 个计划</span><span>{{ results.length }} 条结果</span><span>{{ openAlerts }} 条待处理预警</span><button @click="logout">退出维护模式</button></section>
      <p class="monitor-boundary">当前 Java 批次负责计划治理、结果审阅和告警处置；实际试运行与定时调度仍由 Python 权威运行时执行。</p>
      ...
      <section v-if="tab === 'plans'" class="monitor-grid">
        ...
        <article class="monitor-panel"><header><h2>现有计划</h2><span>下次执行时间由服务端计算</span></header><div v-if="plans.length" class="monitor-cards"><article v-for="plan in plans" :key="plan.plan_id"><div><strong>{{ plan.plan_name }}</strong><code>{{ plan.rule_id }}</code></div><dl><div><dt>频率</dt><dd>{{ plan.frequency }} · {{ plan.run_time }}</dd></div><div><dt>下次执行</dt><dd>{{ fmt(plan.next_run_at) }}</dd></div><div><dt>阈值</dt><dd>环比 {{ plan.mom_threshold_pct }}% / 同比 {{ plan.yoy_threshold_pct }}%</dd></div></dl><button :data-enabled="plan.status === 'enabled'" @click="toggle(plan)">{{ plan.status === 'enabled' ? '停用' : '启用' }}</button></article></div><p v-else>当前医院还没有监控计划。</p></article>
      </section>
      ...
      <section v-if="tab === 'alerts'" class="monitor-panel monitor-wide"><header><h2>指标预警</h2><span>确认与关闭均记录操作者</span></header><div class="monitor-cards alert-cards"><article v-for="alert in alerts" :key="alert.alert_id"><div><strong>{{ alert.conclusion_code }}</strong><code>{{ alert.rule_id }} · {{ alert.alert_id }}</code></div><dl><div><dt>当前值</dt><dd>{{ pct(alert.current_value) }}</dd></div><div><dt>波动</dt><dd>环比 {{ pct(alert.mom_change_rate) }} / 同比 {{ pct(alert.yoy_change_rate) }}</dd></div><div><dt>状态</dt><dd>{{ alert.status }} · {{ alert.diagnose_status }}</dd></div></dl><p><button v-if="alert.status === 'open'" @click="alertAction(alert, 'acknowledge')">确认</button><button v-if="alert.status !== 'closed'" @click="alertAction(alert, 'close')">关闭</button></p></article><p v-if="!alerts.length">暂无指标预警。</p></div></section>
    </template>
  </main>
</template>
