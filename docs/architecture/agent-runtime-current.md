...
flowchart TD
    ...
    FAST --> COMPILE["PlanCompiler + PlanValidator<br/>编译能力、补前置条件、解析时间"]
    PLAN --> COMPILE
    COMPILE --> CTRL["StateController<br/>根据证据选择下一业务能力"]
    CTRL -->|"执行"| DISPATCH["Deterministic Dispatch<br/>生成唯一工具名和完整参数"]
    DISPATCH --> GATE["ToolGateway<br/>权限、Pydantic 参数、超时、重复调用控制"]
    GATE --> TOOLS{"受控领域工具"}
    TOOLS --> RULE["规则、口径、字段映射"]
    TOOLS --> SQL["SQL 对象、只读试运行"]
    TOOLS --> OTHER["诊断、变更预览、Excel 分析"]
    RULE --> EVIDENCE["结构化工具结果与 Evidence"]
    SQL --> EVIDENCE
    OTHER --> EVIDENCE
    EVIDENCE --> VERIFY["PlanVerifier<br/>事实、SQL 链路、周期、数值一致性"]
    VERIFY -->|"仍缺计划内事实"| CTRL
    ...
    class PLAN,FINAL llm;
    class AUTH,SPLIT,SUB,ONE,ENTRY,FAST,COMPILE,CTRL,DISPATCH,VERIFY,CLARIFY,ANSWER,TEMPLATE,GUARD,MERGE,SSE code;
...
| `ToolGateway` | 工具网关 | 工具名、参数、权限、运行状态 | 标准 `ToolResult` | 集中做权限、Pydantic 校验、可用性、超时、缓存和重复调用控制 |
...
1. **类型化边界**：计划、状态、工具参数、工具结果和 Evidence 全部使用 Pydantic，拒绝额外字段。
2. **业务计划编译**：Planner 不输出工具名；Compiler 自动补齐能力依赖和必需事实。
3. **确定性时间解析**：自然月、至今和跨轮时间复用统一转换为左闭右开区间。
4. **复合任务隔离**：2 至 3 个指标分别使用独立运行状态，完成后再确定性合并。
...
8. **证据链校验**：Verifier 核对规则、医院、数据源、统计周期、`sql_id`、结果 ID、上下文摘要和数值公式。
9. **安全回答守卫**：模型不能输出内部工具协议、隐藏思维链、凭据或无证据患者数据。
10. **结构化会话记忆**：指标 ID、多个指标 ID、统计周期、最近运行和上传文件引用独立保存，文本历史只作辅助。
11. **全节点 Trace**：记录中英文节点、类型、耗时、完整安全输入输出、处理说明和配置，敏感字段递归脱敏。
12. **失败分级**：区分用户澄清、业务确认、管理员审批、实施支持、系统运维和安全拒绝；默认最多重规划一次。
