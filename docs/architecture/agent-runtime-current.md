...
候选 profile 的 `effective_to` 等可选治理字段允许为空；校验器保留该 Wiki 语义并仅创建
不可修改视图，避免 Java 不可空集合工厂把合法的长期有效配置误判为运行异常。
...
### AnswerTemplateRegistry 与 Final Answer

最终回答的事实来源仍然只能是 `VerifiedEvidence`。服务端在调用 Final Answer LLM 前，
根据已校验的 `PlanIntent + requested_outputs` 从
`resources/answer-templates/` 选择唯一一份回答或报告模板：

```text
VerifiedEvidence
→ AnswerTemplateRegistry 选择 template_id/version
→ Final Answer LLM 按模板组织 Markdown
→ AnswerContractValidator
   ├─ 通过：返回回答
   └─ 失败：修复一次 → 仍失败则确定性 Evidence 模板
```

- 具体业务模板不进入公共 Prompt；公共 Prompt 只声明必须遵守本轮动态模板。
- 模型每轮只看到一个模板，不会接收所有意图的模板，也不能自行选择版式。
- 契约校验拒绝工具协议、未替换占位符、缺少必需章节以及丢失分子、分母、指标率。
- SQL、候选口径模拟、实施验收和差异归因等高风险回答继续保留确定性渲染路径，
  但使用同一模板编号体系并在 Trace 记录模板版本。
- Vue 使用安全 Markdown 子集渲染，不采用 `v-html`，标题、摘要引用块、表格、列表和
  代码均由 Vue 文本节点输出，模型返回的 HTML 或脚本不会执行。
...
- 结构阻断时不继续访问业务数据。候选口径试算会逐项比较分子、分母和指标率：
  - 分子、分母及所有已提供维度完全一致时，可确认口径原因并短路；
  - 部分维度一致且用户描述或文件字段支持该口径时，标记“高度相关”，保留剩余差异并继续检查记录集合和数据质量；
  - 只有一个未标注数值碰巧一致时，只标记“可能相关”，不能据此确认原因。
- 最终回答始终展示已执行候选口径的分子、分母、指标率、匹配等级及剩余维度差异，避免候选试算结果只存在于 Trace。
