...
- 只响应 `PLAN_INTENT_MISMATCH`、`TASK_TYPE_MISMATCH` 等可重新规划的语义错误。数据库、权限、对象过期、Evidence 冲突和普通工具失败不会触发 Replanner；可从用户原文确定性识别的结果查询误判会在编译前直接纠偏，诊断工具自身失败则立即停止并进入系统操作兜底。
