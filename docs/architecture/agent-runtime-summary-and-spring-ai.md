...
flowchart LR
    ...
    DOMAIN -.->|"仅允许重规划的 FailureClass"| REPLAN["Replanner LLM<br/>最多一次"]
    REPLAN --> IR
    DOMAIN --> EV["证据层<br/>Evidence Ledger -> PlanVerifier"]
    EV --> OUT["回答层<br/>确定性模板或 Final Answer LLM"]
    OUT --> UI

    DRAFT["独立指标草稿 API LLM<br/>不属于主对话链"] -.-> DOMAIN
    DIAG["粘贴 SQL 诊断子链 LLM<br/>按需执行"] -.-> DOMAIN
