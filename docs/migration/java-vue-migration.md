...
> 更新日期：2026-07-22。阶段 0、阶段 1、阶段 2、阶段 3，阶段 4 的 Agent 主链，以及阶段 5 的 Trace、运行观察、元数据和医学术语只读工作台子批次已完成。FastAPI 仍是权威运行时，Java 服务只在 `8766` 影子端口验证兼容性，Vue 开发服务器默认仍调用 `8765` 以维持双栈回退。
...
- 术语概念检索、详情、版本和确定性识别已在阶段 5 子批次迁移；候选、审批、发布和回退仍待迁移，并需在入口切流前使用真实登录会话完成验收。元数据概览与同步也已迁移。
...
- 待迁移：术语候选/审批/发布/回退、审批、实施和监控页面，以及 Vue 构建入 JAR 与正式切流。
