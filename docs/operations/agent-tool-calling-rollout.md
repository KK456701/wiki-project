...
- 每次请求恢复结构化状态和最近 8 轮对话，结构化状态优先于历史文本；
- 内部会话键按医院、用户和公开 `session_id` 三者隔离；
- 当前指标可以跨轮恢复，但定义、公式和本院生效口径必须重新调用工具读取；
- 同一轮第二次同参工具调用复用首次安全结果，缓存只存在当前运行，不跨轮持久化。
...
9. 连续询问“急会诊及时到位率怎么算”和“我想看今年6月到现在的这个指标是多少怎么算”，第二轮仍使用同一指标并只在获得 SQL 校验与试运行证据后回答数值；
10. 两轮“查看链路”均通过登录态 `/api/agent/runs/{trace_id}` 加载，其他医院访问同一 Trace 返回 403；
11. 公式使用普通文本 `分子 ÷ 分母 × 100%`，没有比较证据时只说明口径不同、结果不可直接比较。

前端会在每次发送前重新读取能力开关。新 Agent 尚未开始时若能力检查不可用，会保留稳定流程；一旦收到 `agent_start`，失败后不会自动重放旧请求，以免 SQL 试运行等动作被重复执行。

修改 `config.yaml` 中的 Agent 开关、模式、模型或超时后必须重启 FastAPI。浏览器刷新只会重新读取已经运行进程暴露的能力，不会让服务端重新加载配置文件。
...
- [ ] 同一会话双轮追问、两条登录态 Trace 和 390px 无横向溢出已通过真实浏览器验收。
