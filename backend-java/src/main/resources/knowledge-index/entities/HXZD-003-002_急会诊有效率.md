...
updated: 2026-07-28
type: entity
tags: ["HXZD-003-002", "", "CORE_CONSUL_OUT", "逻辑判定类"]
...
> **指标概念：** [[concepts/HXZD-003-002急会诊有效率]] | **制度：**  | **事件编码：** CORE_CONSUL_OUT
> **版本：** 公版 | **指标导向：** 逐步提高
...
#EQUALS{:syncType; increment; AND (t1.MODIFIED_AT BETWEEN :startTime AND :endTime OR A.MODIFIED_AT BETWEEN :startTime AND :endTime OR t2.MODIFIED_AT BETWEEN :startTime AND :endTime OR C.SIGNED_AT BETWEEN :startTime AND :endTime)}
 #EQUALS{:syncType; single; AND t1.ENCOUNTER_ID in (select ENCOUNTER_ID from OUTPATIENT_ENCOUNTER where SCHEDULED_AT is
```
...
<!-- entity: HXZD-003-002 | code: HXZD-003-002 | system:  | dim: 逻辑判定类 -->
