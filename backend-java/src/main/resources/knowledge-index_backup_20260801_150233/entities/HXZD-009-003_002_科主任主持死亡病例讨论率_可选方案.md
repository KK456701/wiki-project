---
title: "衍生指标实体 - HXZD-009-003_002_科主任主持死亡病例讨论率"
created: 2026-07-29
updated: 2026-07-29
type: entity
tags: ["HXZD-009-003_002", "死亡病例讨论制度", "CORE_DEATH_EXT", "逻辑判定类"]
sources: [raw/excel/AI指标顾问【AI Metrics Advisor】7.xls]
confidence: high
---

# HXZD-009-003_002 — 科主任主持死亡病例讨论率_科主任数据来源：统一配置行政职务包含“主任”

> **指标概念：** [[concepts/HXZD-009-003科主任主持死亡病例讨论率]] | **制度：** 死亡病例讨论制度 | **事件编码：** CORE_DEATH_EXT
> **版本：** 公版 | **指标导向：** 逐步提高
...
| 目标值 | 0.95 |
| 异常范围初筛 | < 60% |
| 方案类型 | 可选方案（科主任数据来源：统一配置行政职务包含“主任”） |
| 配置类型 |  |
| 配置值 |  |
...
DateSequence AS (
    -- 生成1-30的数字序列（死亡后第1天到第30天，SQL Server 递归 CTE）
    ...
    SELECT 
        ...
        (
            ...
            FROM (
                ...
                WHERE 
                    -- 1900-01-01 为星期一，固定余数 5/6 表示周六/周日，不依赖 @@DATEFIRST
                    ((DATEDIFF(DAY, CONVERT(date, '19000101', 112),
                        CAST(DATEADD(DAY, ds.day_offset, dc.DEATH_DATE) AS date)) % 7 + 7) % 7) NOT IN (5, 6)
                    -- 排除节假日
                    AND NOT EXISTS (
                        SELECT 1 
                        FROM HOLIDAY h 
...
- 血缘解析：[[entities/HXZD-009-003_002_科主任主持死亡病例讨论率_血缘说明.md]]
...
<!-- entity: HXZD-009-003_002 | code: HXZD-009-003 | system: 死亡病例讨论制度 | dim: 逻辑判定类 -->
