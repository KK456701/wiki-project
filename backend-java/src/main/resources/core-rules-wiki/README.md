...
指针完成。真正包含医院口径差异的 release 必须记录依赖的公司基础版本，公司升级
不会静默覆盖医院口径。

当前 `hospital_001` 指针指向的
`KB-20260726-HOSPITAL001-DUALDB` 仅是双库执行契约覆盖层：来源仍是公司原始资料，
`hospital_overrides` 为空，只补充数据库角色、SQL结果列和验证状态。目录沿用
`releases/hospitals/` 是为了运行兼容，不表示医院提供了独立口径。未来只有由医院
差异资料或回收包生成、人工审核且覆盖索引非空的发布，才称为医院差异知识版本。
