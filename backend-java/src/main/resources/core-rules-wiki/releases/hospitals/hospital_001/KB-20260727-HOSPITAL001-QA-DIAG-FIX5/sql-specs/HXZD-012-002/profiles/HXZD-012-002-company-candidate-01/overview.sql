...
tt."分子-四级手术患者死亡率",
tt."分母-三级手术患者死亡率",
CONCAT(tt."分子-四级手术患者死亡率", ' : ', tt."分母-三级手术患者死亡率") as 监测情况,
CASE
WHEN tt.third_grade_rate IS NULL OR tt.third_grade_rate = 0 THEN NULL
ELSE tt.fourth_grade_rate / NULLIF(tt.third_grade_rate, 0)
END AS index_value
from (
select
...
CASE WHEN a.四级手术例数 = 0 THEN NULL ELSE 1.0 * a.四级手术且死亡患者人数 / a.四级手术例数 END AS fourth_grade_rate,
CASE WHEN b.三级手术例数 = 0 THEN NULL ELSE 1.0 * b.三级手术且死亡患者人数 / b.三级手术例数 END AS third_grade_rate
 from (
SELECT
