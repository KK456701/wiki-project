...
select
CASE WHEN tt.四级手术并发症发生率 IS NULL THEN '无数据'
     ELSE CONCAT(CONVERT(numeric(19,2), tt.四级手术并发症发生率), '%') END
     as "分子-四级手术并发症发生率",
CASE WHEN tt.三级手术并发症发生率 IS NULL THEN '无数据'
     ELSE CONCAT(CONVERT(numeric(19,2), tt.三级手术并发症发生率), '%') END
     as "分母-三级手术并发症发生率",
CASE WHEN tt.四级手术并发症发生率 IS NULL
          OR tt.三级手术并发症发生率 IS NULL
          OR tt.三级手术并发症发生率 = 0
     THEN NULL
     ELSE tt.四级手术并发症发生率 / tt.三级手术并发症发生率
END as index_value,
CONCAT(
    CASE WHEN tt.四级手术并发症发生率 IS NULL THEN '无数据'
         ELSE CONCAT(CONVERT(numeric(19,2), tt.四级手术并发症发生率), '%') END,
...
from (
select
CASE WHEN a.四级手术患者数 = 0 THEN NULL
     ELSE 1.0 * a.四级手术并发症患者数 / a.四级手术患者数 * 100 END
     as 四级手术并发症发生率,
CASE WHEN b.三级手术患者数 = 0 THEN NULL
     ELSE 1.0 * b.三级手术并发症患者数 / b.三级手术患者数 * 100 END
     as 三级手术并发症发生率
 from (
SELECT
