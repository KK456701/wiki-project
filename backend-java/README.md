...
应用本身还有第二道门禁：设置 `MIGRATION_AUTHORITY_RUNTIME=java` 时必须同时设置 `MIGRATION_CUTOVER_APPROVED=true` 和非空 `MIGRATION_READINESS_REPORT_ID`，否则 Spring Boot 拒绝启动。默认配置始终为 Python 权威、Java 影子。
...
后续按照 `docs/migration/java-vue-migration.md` 在真实环境运行完整验收，再由操作者显式切流。代码提交本身不会改变当前 FastAPI 权威入口。
