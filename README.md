...
第六批前端改造采用“业务页面为主、AI 对话为辅”的工作台结构。登录后会直接进入 `#/monitoring` 指标运算监控页，不再先显示全屏聊天页，也不需要从弹窗打开监控功能。

- 左侧业务导航只展示已经完成并可正常操作的页面，当前首个正式页面为“指标运算监控”。
- 顶部“AI 助手”可随时打开右侧对话抽屉；关闭抽屉不会清空当前会话、输入内容或工作台页面状态。
- “系统自检”和“恢复中心”继续保留为全局运维入口。
- AI 指标自助配置、智能异常排查、口径规则管理、元数据与结构监控四个页面会在第六批后续子批次逐页迁移；页面完成前不显示不可操作的空白导航项。

现有“指标设计稿”“审批与版本”“数据库与元数据”等入口暂时位于兼容工具区，后续会迁入对应的正式业务页面。
...
1. 使用医院人员身份进入系统，页面会直接进入 `#/monitoring`，确认顶部医院编号正确。
2. 首次进入需要管理员权限的监控数据时完成管理员登录，登录后会自动返回当前监控页面，不会跳转到审批功能。
...
7. 运行失败时根据页面提示前往“恢复中心”重试。
8. 需要询问指标口径或生成 SQL 时，点击顶部“AI 助手”；完成问答后关闭抽屉即可继续处理原监控页面。
