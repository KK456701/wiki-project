...
- **会话记忆**：用户问题和系统回答完整写入 SQLite 与 JSONL，便于审计与故障恢复；运行时只向本地小模型传递当前指标、医院、统计区间等结构化工作记忆，不把整段历史对话重复塞入上下文。用户本轮明确给出的统计区间优先于上轮记忆，短指令“生成 SQL”“试运行”会继承最近一次有效区间，旧会话也可从最近的用户问题恢复日期。
