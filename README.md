...
```text
本院生效口径 -> 分子/分母业务定义 -> 数据库表和医院字段 -> 本次参数 -> 技术 SQL
```

- “当前采用口径”会明确显示标准口径或本院生效口径。本院修改过阈值、分子、分母或筛选条件时，同时显示本院值、标准值和对应版本。
- “为什么这样计算”用表格说明分母纳入哪些业务数据、分子在分母基础上还需满足什么条件，以及最终公式。
- “从哪里取数”显示业务数据库、主表、业务字段含义和医院实际字段，方便实施人员核对字段映射。
...
现有旧 SQL 如果尚未返回分子、分母列，系统仍可显示指标值，但会提示重新生成新版 SQL 后再查看完整计算过程。
