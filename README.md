...
- **全阶段运行 Trace**：成功和失败消息都可打开“查看链路”；认证后的 `/api/agent/runs/{trace_id}` 返回会话读取、Planner、计划编译校验、状态控制、Executor、工具、Verifier、回答守卫和会话保存节点，并展示完整安全输入输出、数据处理、节点配置与耗时。密码、令牌、连接串、患者行级明细和隐藏思维过程始终不记录。
