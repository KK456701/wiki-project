...
- **证据链与运行 Trace**：对话页展示本轮实际调用的工具摘要；认证后的 `/api/agent/runs/{trace_id}` 返回计划、工具、校验与结束状态，参数、患者明细和思维过程不会进入公开链路。
...
7. 模型只根据当前证据组织中文回答，并通过 SSE 输出工具摘要、答案和 Trace ID。
...
- Agent 对话链路来自认证后的 `/api/agent/runs/{trace_id}`，展示业务计划、实际工具步骤、校验状态和结束原因；不会返回模型思考过程、密钥、患者明细或完整工具参数。
...
- Agent 对话运行数据由 `AgentRunTraceStore` 保存；指标实施等非对话业务流程继续使用各自的工作流 manifest 与 `TraceRecorder`。
