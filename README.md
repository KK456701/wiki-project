...
- **公司业务库真实取数**：`hospital_001` 默认连接 `WIN60_QA_991827`；已核验的急会诊指标从 `WINDBA.INPATIENT_CONSULT_APPLY` 与 `WINDBA.INP_CONSULT_INVITATION` 取数，分子、分母、明细预览和 Excel 导出共用同一份统计范围与字段映射。
...
- **会话记忆**：用户问题和系统回答完整写入 SQLite 与 JSONL，便于审计与故障恢复；运行时只向本地小模型传递当前指标、医院、统计区间等结构化工作记忆，不把整段历史对话重复塞入上下文。用户本轮明确给出的统计区间优先于上轮记忆，也可只修改开始日期或结束日期并沿用另一端；短指令“生成 SQL”“试运行”会继承最近一次有效区间，旧会话也可从最近的用户问题恢复日期。统计时间调整属于查询参数，不会进入医院口径审批。
...
| 指标编码 | 指标名称 | 公司库适配状态 |
|---|---|---|
| `MQSI2025_001` | 患者入院 48 小时内转科比例 | 待 WxP 与实际库核验字段，禁止生成正式 SQL |
