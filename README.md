...
生成 SQL 后，回答默认面向医生解释业务计算。数据库位置、字段映射、安全校验和 SQL 统一放入默认关闭的“查看技术详情（供信息科和实施人员）”：

```text
当前规则 -> 本次统计哪些记录（分母） -> 哪些记录达到要求（分子） -> 计算结果 -> 折叠技术详情
```

- “当前采用什么规则”会说明指标名称、统计医院、统计时间以及采用标准口径还是本院生效口径。
- “本次统计哪些记录（分母）”使用自然语言说明哪些业务记录进入统计范围、每条记录如何计数。
- “哪些记录达到要求（分子）”先说明分子来自分母，再说明需要满足的额外条件。例如急会诊在本院规定的20分钟内到位才进入分子。
...
2. 继续输入“生成 SQL”。默认区域应说明本次统计哪些急会诊、哪些急会诊算作及时到位，以及分子只从分母中继续筛选。
3. 默认区域应显示本院采用20分钟、标准值为10分钟，并注明“只影响分子，不改变分母”；默认区域不应出现表名、字段名或 SQL。
4. 点击“查看技术详情（供信息科和实施人员）”。展开后应显示 `consult_record.hospital_id`、`consult_record.request_time`、`consult_record.consult_type`、`consult_record.arrive_time` 及完整 SQL。
