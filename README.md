...
- **统计周期与结果意图确定性解析**：用户本轮明确说“从一月份到三月份”等时间范围时，由服务端统一解析为左闭右开区间（2026-01-01 至 2026-04-01）；本轮未提供时间时优先复用上一轮已确认的结构化周期。对于“怎么算，从一月份到现在”这类同时包含可解析周期和实际计算诉求的问法，即使 Planner 误判为公式解释，服务端也会纠偏为指标试运行。Planner 自行生成的起止日期不能覆盖用户原文或已确认状态，因此 4B、8B 与 DeepSeek 使用同一统计边界。
