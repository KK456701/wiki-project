...
生成 SQL 后，回答优先面向医生和医院实施人员解释业务计算，再在末尾展示技术 SQL：

```text
本院生效口径 -> 分母字段关系 -> 分子追加条件 -> 本院口径落点 -> 聚合结果 -> 技术 SQL
```

- “当前采用口径”会明确显示标准口径或本院生效口径。本院修改过阈值、分子、分母或筛选条件时，同时显示本院值、标准值和对应版本。
- “分母如何取数”逐条显示统计范围、分母条件、业务字段、医院表字段、条件来源，以及满足条件后为什么进入分母。
- “分子如何从分母中筛选”先明确分子继承全部分母条件，再显示追加条件。派生字段会展开来源，例如急会诊耗时明确为 `consult_record.arrive_time` 减 `consult_record.request_time` 后换算为分钟。
- “本院口径作用在哪里”显示本院值、标准值、对应条件、医院字段和影响范围；例如本院20分钟只改变分子，不改变同期急会诊总次数这个分母。
- SQL 试运行一次返回 `index_value`、`numerator_count`、`denominator_count` 和兼容字段 `sample_count`，不会额外执行分子、分母查询。
- 例如分子为8、分母为10时，回答会明确说明 `8 / 10 x 100% = 80%`，并列出未进入分子的2条聚合数量。
- 分母为0时显示“本期没有符合分母条件的数据，指标暂不可计算”，不把0%误解为真实业务水平。
- 对话和执行链路只展示聚合数量、统计区间、口径版本和数据源，不展示患者姓名、病历号、业务明细或绑定参数后的 SQL。

现有旧 SQL 如果尚未返回分子、分母列，系统仍可显示指标值，但会提示重新生成新版 SQL 后再查看完整计算过程。旧规则没有结构化字段关系时，回答明确显示“字段关系尚未结构化”，不会根据表名或字段名猜测分子、分母关系。
...
2. 继续输入“生成 SQL”。回答应依次出现“分母如何取数”“分子如何从分母中筛选”“本院口径作用在哪里”。
3. 分母应显示 `consult_record.hospital_id`、`consult_record.request_time` 和 `consult_record.consult_type`；分子应先继承分母，再使用 `consult_record.request_time` 与 `consult_record.arrive_time` 计算耗时。
4. 本院口径表应显示20分钟、标准值10分钟，并注明“只改变分子，不改变分母”。
5. 输入“试运行”。回答应显示本期分子、分母、指标值和计算式；点击“查看链路”，试运行节点只能看到聚合结果、统计区间、数据源、耗时和运行 ID，不应出现患者明细或绑定后的 SQL。
