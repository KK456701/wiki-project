...
本项目是一个面向医院核心制度指标的本地化 Agent。`core-rules-wiki/` 是指标制度、本院口径、字段映射、SQL 规格和版本的权威知识源；会话、审计、Trace、Evidence、SQL 对象和监控计划保存在内嵌 SQLite。部署不再需要 MySQL。

项目默认适配轻量本地部署：Java 单 JAR 或 FastAPI 回退栈 + Wiki 目录 + SQLite 单文件 + DBHub sidecar + Ollama/DeepSeek。患者业务数据仍只通过 DBHub 只读访问 SQL Server，不进入 Wiki、SQLite、Trace 或模型上下文。
...
- **轻量 Evidence Ledger**：工具成功结果先生成未验证 `EvidenceEnvelope`，Verifier 独立写入验证记录。默认写入 SQLite，不可用时降级到 `runtime/agent_evidence.jsonl`；不保存 SQL 原文或患者行级数据。
...
- **全阶段运行 Trace 与运行观察**：成功和失败消息都可打开“查看链路”；认证后的 `/api/agent/runs/{trace_id}` 返回父子关系、子任务泳道、真实时间偏移、节点类型、模型与 Token、缓存、重试、FailureClass、版本和 Evidence 来源。规则命中与本地语义唯一命中时不会额外调用指标消歧 LLM；复合请求可确定性生成子计划时也会跳过 Planner，但每个实际调用的 `final_answer_llm` 都独立落在对应子任务泳道。页面提供瀑布图、调用树、筛选、最慢节点和证据定位；“Agent 运行观察”通过 `/api/agent/runs` 与 `/api/agent/runs/metrics` 展示当前医院的请求量、成功率、p50/p95/p99、工具/模型性能和轻量阈值提示。运行摘要保存在内嵌 SQLite，不需要外部观测中间件。
...
- **Wiki + SQLite 轻量化迁移**：规则检索、医院生效口径、字段映射、SQL 规格和不可变版本已从 MySQL 全部迁移到 `core-rules-wiki/`；会话、权限、审计、Trace、Evidence、SQL 对象、监控和术语状态迁移到 `runtime/wiki_agent_runtime.db`。Python 与 Java 共用这两个本地数据源，部署不再安装 MySQL。迁移脚本已对旧库 42 张表、78,350 行完成行数核对和 SQLite 完整性检查。相对时间解析同时改为用户原文优先，Qwen 8B 不能再用模型虚构的 2025 日期覆盖“从一月份到现在”的 2026 统计边界。

## 技术栈

- 当前权威后端：Java 17、Spring Boot 4.1、Spring AI 2.0；FastAPI、Pydantic 和 SQLAlchemy 作为回退栈保留。两套运行时共用 Wiki 规则与 SQLite 运行库；医院业务源为 SQL Server，只通过 DBHub 只读访问
...
- SQL 模板：Jinja2
- 知识库：Markdown、YAML 和 JSON 索引保存已审核规则、术语、医院覆盖及版本；SQLite 只保存可变运行状态
...
| 数据或组件 | 当前职责 | 边界 |
|---|---|---|
| `core-rules-wiki/` | 制度规则、本院口径、字段映射、SQL 规格、术语与不可变版本 | 系统权威知识库；变更必须经预览和审批 |
| `runtime/wiki_agent_runtime.db` | 会话、权限、审计、Trace、Evidence、元数据快照、诊断、监控和导出索引 | 内嵌 SQLite，不包含规则正文或患者原始行 |
| 医院业务库 | 患者业务记录和指标计算原始数据 | 主服务通过 DBHub 只读访问；数据不出院 |
| 公司包交换记录 | 公司候选、标准版本和发布包的运行状态 | 保存在同一 SQLite 中并按医院/包 ID 隔离，不保存患者明细 |
| SQLite Evidence 表 / JSONL | Evidence Envelope 与独立验证记录 | 只保存安全允许列表和对象引用；SQLite 不可用时 JSONL 兜底 |
...
当前部署不需要安装 MySQL。规则、医院口径、字段映射和 SQL 规格直接读取 `core-rules-wiki/`；可变运行数据由 Java 与 Python 共用一个内嵌 SQLite 文件：

```yaml
runtime_db_url: "sqlite+pysqlite:///runtime/wiki_agent_runtime.db"
company_db_url: "sqlite+pysqlite:///runtime/wiki_agent_runtime.db"
...
首次启动 Java 或 FastAPI 时会自动创建 `runtime/wiki_agent_runtime.db` 和所需表，不需要手工建库。医院患者业务数据不复制到 SQLite，正式查询仍由 DBHub 使用医院只读账号访问 SQL Server。

从旧 MySQL 部署升级时，可一次性迁移会话、权限、审计、Trace、Evidence、SQL 对象、监控与术语等运行数据：

```powershell
python scripts\migrate_mysql_to_sqlite.py --switch-config
```

迁移脚本会核对每张表的行数并执行 SQLite 完整性检查；迁移成功后应用不再连接旧 MySQL。旧库只作为人工回退备份保留，确认稳定后可自行归档。
...
如果 SQLite 运行库来自旧版本，拉取新版本后可再执行一次幂等结构迁移：
...
首批四个指标无需导入数据库，Java 与 Python 会直接读取 Wiki 索引、医院 override、字段映射和 SQL 规格。当前落地状态为：
...
公司包交换、候选审批和发布状态同样写入内嵌 SQLite；正式规则正文和不可变版本仍落到 Wiki 文件并重建索引。无需额外的公司数据库服务。
...
语料当前覆盖 35 个核心制度指标及共用诊断、科室、人员角色、时间范围和数据值概念。`core-rules-wiki/terminology/core_indicator_terms.yaml` 和 Wiki 版本文件是审核后内容的权威来源；SQLite 保存候选、审批状态和审计记录。
...
6. 工具从 Wiki 读取本院生效口径、字段映射和 SQL 规格，必要时经 DBHub 准备并试运行只读 SQL。
...
Planner JSON 形状错误时最多修复一次；执行方向错误时最多重规划一次。缺少时间、指标候选不唯一或业务确认时返回明确澄清，不再切换到旧聊天流程。生效口径接口和执行链路统一返回 `rule_source: wiki`；索引或必要文件缺失时失败关闭，不会回退到旧 MySQL 形成第二事实源。
...
2. 业务字段字典说明 `request_time`、`arrive_time` 等字段在指标中的业务含义和期望类型，直接保存在可审阅的 Wiki/YAML。
3. `core-rules-wiki/hospital-mappings/<hospital_id>/` 保存每家医院的业务字段到实际数据库表字段映射，只有状态为 `confirmed` 的映射可以生成可执行 SQL。
...
确认 DBHub 和当前 Java/FastAPI 服务均已启动后执行（无需 MySQL）：
...
Wiki 是规则唯一事实源，不再存在 MySQL 规则主表或故障兜底分支。国标、公司标准、本院 override、不可变版本、字段映射、SQL 规格和术语均保存在 `core-rules-wiki/`，Java 与 Python 使用同一套确定性解析器访问。

### 为什么选择 Wiki + SQLite

当前目标是医院单机轻量部署，因此把“可版本控制的规则事实”和“高频可变的运行状态”分开：

| 能力 | Wiki / YAML / JSON | 内嵌 SQLite | 本地模型 |
|---|---|---|---|
| 国标、医院差异、不可变版本和有效期 | 权威存储 | 保存审批游标与审计 | 不直接写入 |
| 字段映射、结构化计算定义和 SQL 规格 | 权威存储 | 保存临时 SQL 对象引用 | 可辅助生成草稿 |
| 制度原文、实施知识和审核术语 | 权威存储 | 保存候选与发布状态 | 负责理解、摘要和候选消歧 |
| 会话、Trace、Evidence、监控和权限 | 不保存 | 权威运行存储 | 只消费允许的安全上下文 |
| 患者业务数据 | 不保存 | 不保存 | 不接触患者行 |

- **轻量部署**：SQLite 与单 JAR/FastAPI 同机运行，没有数据库服务、账号、端口或额外运维。
- **规则可审阅**：每条规则和医院映射都是可读文件，可通过 Git 审阅、签名包交换和不可变版本恢复。
- **访问确定性**：运行时读取索引和结构化文件，不让模型从长文档猜测当前版本、字段或 SQL。
- **写入受控**：审批先写临时状态，再以原子文件替换发布 Wiki 版本；失败不会把半成品标成生效。
- **双栈一致**：Java 和 Python 指向相同 Wiki 根目录与 SQLite 文件，不需要做跨数据库同步。
- **SQL 安全**：SQL 仍由固定规格和医院映射编译，经安全校验后只通过 DBHub 访问 SQL Server。
...
医院口径审批和版本恢复会生成新的不可变 Wiki 版本并更新索引；医院知识包、公司回收和发布也以 Wiki 文件为规则载体。手工修改或发布后应重建索引，应用启动时会检查缺失索引和非法版本引用。
...
本项目强调“结构化规则事实优先，LLM 辅助表达”。Wiki 是规则、公式、字段映射、SQL 规格和医院覆盖的唯一权威来源，SQLite 只保存可变运行状态。任何规则变更都应通过审批、索引校验和测试后再提交。
