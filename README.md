...
- **复合多指标查询**：用户使用“还有”“以及”“另外”等明确连接词一次查询 2 至 3 个指标时，服务端先确定性拆分为相互隔离的指标子任务，并只解析一次公共统计周期。带明确周期的结果查询由服务端清理“的结果”等省略措辞并直接生成各子任务业务计划，不依赖小模型补全；检索同时命中正式指标和名称含“测试”的指标时优先唯一正式候选，多个正式候选仍要求用户确认。完成后会在安全会话状态中保存本轮全部 `rule_id`，后续“这两个的 SQL”“它们的结果”“这两个的分子分母是什么意思”“分别解释公式”等复数指代会再次确定性拆分，按原统计周期独立执行并合并；SQL 标题及分子分母说明均取自本轮生效规则证据，不使用 Planner 或历史文本补充口径结论。
...
- **全阶段运行 Trace 与运行观察**：成功和失败消息都可打开“查看链路”；认证后的 `/api/agent/runs/{trace_id}` 返回父子关系、子任务泳道、真实时间偏移、节点类型、模型与 Token、缓存、重试、FailureClass、版本和 Evidence 来源。页面提供瀑布图、调用树、筛选、最慢节点和证据定位；“Agent 运行观察”通过 `/api/agent/runs` 与 `/api/agent/runs/metrics` 展示当前医院的请求量、成功率、p50/p95/p99、工具/模型性能和轻量阈值提示。全部使用现有 MySQL/JSONL 与原生 HTML/CSS/JavaScript。
...
- **最终回答协议防护**：Final Answer 模型没有工具权限；如果模型仍把 DSML、`tool_calls`、`invoke` 或其他内部工具协议写入正文，ResponseGuard 会阻止展示并最多纠正一次，重复异常只返回安全错误，不会执行模型虚构的工具。
...
浏览器模型选择器 -> POST /api/agent/chat/stream -> 登录与医院权限
       -> AgentRuntimeService -> CompoundRequestSplitter（单指标直通；复合请求拆为 2 至 3 个隔离子任务）
