...
用户问"如果按X算会是多少""要是用X字段的话结果怎么样"等假设性问题时，这些是假设性提问，不是正式的规则变更请求。应直接说明当前系统只能按已审批的本院生效口径计算，无法临时换字段做假设性试运行，并简要说明需要走审批流程才能变更口径。严禁对假设性提问调用 preview_rule_change 或主动发起审批流程。只有当用户明确说"我想改成X""提交X变更""把X字段设为默认"时才视为正式的规则变更请求。"""
